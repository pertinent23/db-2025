...
CREATE OR REPLACE VIEW AVG_TIME_ITINERAIRE AS
(
    SELECT ITINERAIRE_ID, TRAJET_ID,
    SEC_TO_TIME(AVG(TIMESTAMPDIFF(SECOND, HEURE_DEPART, HEURE_ARRIVEE))) AS AVG_STOP_TIME
    FROM HORRAIRE
    WHERE HEURE_ARRIVEE IS NOT NULL AND HEURE_DEPART IS NOT NULL
    GROUP BY ITINERAIRE_ID, TRAJET_ID WITH ROLLUP
);

CREATE OR REPLACE VIEW ARRET_TRAINS_PAR_SERVICE AS
(
    SELECT
        a.ID AS ARRET_ID,
        a.NOM AS ARRET_NOM,
        t.SERVICE_ID,
        COUNT(CASE WHEN h.HEURE_ARRIVEE IS NOT NULL THEN 1 END) AS TOTAL_ARRIVEES,
        COUNT(CASE WHEN h.HEURE_DEPART IS NOT NULL THEN 1 END) AS TOTAL_DEPARTS
    FROM ARRET a
    LEFT JOIN HORRAIRE h ON a.ID = h.ARRET_ID
    LEFT JOIN TRAJET t ON h.TRAJET_ID = t.TRAJET_ID
    GROUP BY a.ID, a.NOM, t.SERVICE_ID
    ORDER BY TOTAL_ARRIVEES + TOTAL_DEPARTS DESC
);
